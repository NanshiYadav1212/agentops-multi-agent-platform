...
# Gracefully handle pytesseract import
try:
    import pytesseract
...
async def process_ocr(content: bytes, filename: str) -> tuple[list[str], str]:
    ...
    if not PYTESSERACT_AVAILABLE:
        raise ValueError("pytesseract is not installed on the server environment.")
    ...
    try:
        ...
        try:
            ...
            log.error("tesseract_binary_missing_or_failed", error=str(ocr_err))
            raise ValueError("Tesseract binary engine is not installed or accessible on this host environment.")
