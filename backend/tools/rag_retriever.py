...
def _get_collection():
    """
    Lazy loader for ChromaDB and SentenceTransformer embedding function.
    Initializes objects on first tool call/use to avoid RAM spikes on app startup.
    """
...
def rag_tool(
    ...
    try:
        ...
        # Column-name-aware query expansion for tabular data
        ...
        # Match year tokens as they appear in CSV chunks (e.g. "1990.0")
        ...
        # Increase TOP_K for CSV files to get more candidates
        ...
        # Widen MAX_GAP for CSV files so nearby chunks aren't cut off
        ...
        similarity = max(
            ...
            min(
                100,
                round((2.0 - best_distance) / 2.0 * 100)
            )
        )
